...
A `FormGroupElement` can also be created via the `BootstrapViewElements.bootstrap.builders#formGroup` factory method.
...
----
BootstrapViewElements.bootstrap.builders.formGroup()
                   .build();
----
...
----
BootstrapViewElements.bootstrap.builders.formGroup()
                   .label( BootstrapViewElements.bootstrap.builders.label( "Author" )
...
----
<1> If an object is bound to the form and it contains validation errors, the form group will try to detect whether field errors are present for the current control.
    Should one or more errors be available, they will be rendered in the form group.
